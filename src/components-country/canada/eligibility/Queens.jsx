...
import { 
  ...
  TrendingUp, Award, Users, Sparkles, Star, Zap, GraduationCap, Crown, MapPin
} from 'lucide-react';
  ...
  const sections = [
    {
      ...
      content: [
        { label: "Undergraduate", detail: "Minimum 80% cumulative average; highly competitive programs 90%+" },
        { label: "Prerequisites", detail: "ENG4U + 5 additional 4U/M courses (Calculus for Commerce/Eng)." },
        { label: "Graduate Programs", detail: "Minimum B+ (77%) standing in the final two years of study." }
      ]
    },
    {
      ...
      content: [
        { label: "IELTS / TOEFL", detail: "6.5 (no band < 6.0) / 88 overall (24W, 22S, 22R, 20L)" },
        { label: "PTE Academic", detail: "Minimum overall score of 60." },
        { label: "Duolingo", detail: "110+ (Check program-specific requirements)" }
      ]
    }
  ];

  const stats = [
    { icon: <Users />, label: "Retention Rate", value: "94.7%", color: "bg-blue-800" },
    { icon: <Crown />, label: "National Ranking", value: "Top 5", color: "bg-yellow-600" },
    { icon: <TrendingUp />, label: "Alumni Network", value: "150k+", color: "bg-red-700" },
    { icon: <Award />, label: "Global Impact", value: "Top 10", color: "bg-slate-800" }
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-500 pb-20 overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        ...
        <motion.div 
          ...
          className="absolute bottom-20 left-20 w-[500px] h-[500px] bg-red-500/10 dark:bg-red-900/10 rounded-full blur-3xl"
        />
      </div>
      ...
      <motion.div style={{ opacity, scale }} className="relative h-[100vh] w-full overflow-hidden">
        ...
        >
          <img 
            src="/assets/colleges/queens.jpg" 
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.8 }} className="relative">
            ...
            <h1 className="text-6xl md:text-8xl font-black text-white mb-4 tracking-tight">Queen's</h1>
            <p className="text-xl md:text-2xl text-white/90 font-medium max-w-2xl mx-auto">The Limestone Legacy of Kingston</p>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-32 space-y-6">
              <motion.div variants={itemVariants} className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl p-8 rounded-[3rem] border border-blue-100 dark:border-slate-800 shadow-2xl relative overflow-hidden group">
                ...
                <motion.div variants={floatVariants} animate="animate" className="w-16 h-16 bg-gradient-to-br from-blue-700 to-blue-950 rounded-2xl mb-6 flex items-center justify-center shadow-xl rotate-3 relative">
                  <Landmark size={32} className="text-white relative z-10" />
                </motion.div>
                
                <h1 className="text-4xl font-black text-slate-900 dark:text-white mb-2 leading-tight">Queen's <span className="text-blue-700 italic block">University</span></h1>
                ...
                <div className="space-y-4 pt-6 border-t border-blue-50 dark:border-slate-800">
                  <div className="flex justify-between items-center group/deadline">
                    <span className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 group-hover/deadline:text-blue-600 transition-colors">
                      <Clock size={14} className="text-blue-600" /> OUAC Deadline
                    </span>
                    <span className="font-bold dark:text-white">January 15</span>
                  </div>
                  <div className="flex justify-between items-center group/deadline">
                    <span className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 group-hover/deadline:text-blue-600 transition-colors">
                      <Clock size={14} className="text-blue-600" /> PSE Deadline
                    </span>
              ...
              <motion.div variants={itemVariants} className="relative bg-gradient-to-br from-blue-900 via-slate-900 to-black p-8 rounded-[2.5rem] text-white shadow-2xl overflow-hidden group">
                ...
                <p className="text-sm font-medium leading-relaxed italic relative z-10">
                  "Queen’s provides a tight-knit community and a transformative student experience that balances academic rigour with social leadership."
                </p>
              </motion.div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="grid md:grid-cols-2 gap-6">
              {sections.map((section, idx) => (
                <motion.div key={idx} variants={itemVariants} whileHover={{ y: -8, rotateY: 2 }} className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-blue-50 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden group">
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-500/0 via-red-500/0 to-yellow-500/0 group-hover:from-blue-500/5 group-hover:via-red-500/5 group-hover:to-yellow-500/5 transition-all duration-500" />
                  ...
                  <div className="space-y-4 relative z-10">
                    {section.content.map((item, i) => (
                      <motion.div key={i} whileHover={{ x: 5 }} className="group/item">
                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-tighter group-hover/item:text-blue-600 transition-colors flex items-center gap-2">
            ...
            <motion.div variants={itemVariants} className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden border border-slate-800">
              <div className="absolute inset-0 opacity-5" style={{ backgroundImage: "linear-gradient(rgba(37, 99, 235, 0.2) 1px, transparent 1px), linear-gradient(90deg, rgba(37, 99, 235, 0.2) 1px, transparent 1px)", backgroundSize: "50px 50px" }} />
              <h2 className="text-3xl font-black mb-8 flex items-center gap-3 relative z-10"><FileText className="text-blue-400" /> Application Checklist</h2>
              <div className="grid md:grid-cols-2 gap-4 relative z-10">
                {["OUAC Portal (101/105)", "Personal Statement (PSE)", "English Language Proof", "Verified Transcripts", "Reference Letters (Grad)", "Study Permit (Intl)"].map((item, i) => (
                  <motion.div key={i} whileHover={{ x: 10, scale: 1.02 }} className="flex items-center gap-3 bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-white/10 hover:border-blue-500/50 transition-all group">
                    <CheckCircle2 size={18} className="text-blue-400 group-hover:text-yellow-400" />
            ...
            <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-blue-50 dark:border-slate-800 shadow-sm relative overflow-hidden">
              ...
              <div className="grid md:grid-cols-2 gap-8 items-center relative z-10">
                <div className="space-y-4">
                  {["Critical Reflection", "Co-Curricular Engagement", "Strong Writing Skills", "Global Mindset"].map((rule, i) => (
                    <motion.div key={i} whileHover={{ x: 10 }} className="flex items-center gap-4 group">
                      <div className="w-2 h-2 rounded-full bg-blue-600 group-hover:bg-yellow-500" />
                      <span className="text-md font-bold text-slate-700 dark:text-slate-300 group-hover:text-blue-700">{rule}</span>
                    </motion.div>
                  ))}
                </div>
                <motion.div whileHover={{ scale: 1.05 }} className="bg-gradient-to-br from-blue-900 via-slate-900 to-black p-8 rounded-[2rem] text-white shadow-2xl relative overflow-hidden">
                  <p className="text-xs font-black uppercase tracking-widest mb-2 opacity-90 relative z-10">Financials</p>
                  <h4 className="text-xl font-black mb-2 relative z-10 flex items-center gap-2"><Award size={24} /> Scholarships</h4>
                  <p className="text-sm font-medium leading-relaxed relative z-10">
                    Merit-based entrance awards ranging from $2k to $80k (Chancellor's Scholarship) for top academic achievers.
                  </p>
                </motion.div>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="pt-10 flex flex-col items-center gap-6">
              ...
              >
                <motion.span initial={{ x: "-100%" }} whileHover={{ x: "100%" }} transition={{ duration: 0.6 }} className="absolute inset-0 bg-gradient-to-r from-blue-600 via-blue-700 to-blue-600" />
                <span className="relative z-10 flex items-center gap-2"><Sparkles size={18} /> Apply to Queen's</span>
              </motion.button>
              <motion.p animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }} className="text-slate-400 font-bold text-xs uppercase tracking-widest flex items-center gap-2">
                <Clock size={14} className="text-blue-600" /> Fall 2026 Admissions Now Open
              </motion.p>
            </motion.div>
