  ...
  const sections = [
    {
      ...
      content: [
        { label: "Undergraduate", detail: "University entrance qualification (Abitur/IB) with strong math focus." },
        { label: "Eignungsprüfung", detail: "Internal aptitude assessment for Engineering and CS programs." },
        { label: "Graduate Programs", detail: "B.Sc. in relevant field with ECTS compatibility (German grade 2.5+)." }
      ]
    },
    {
      ...
      content: [
        { label: "German Proficiency", detail: "DSH-2, TestDaF 4x4, or Goethe C2 (German-taught programs)." },
        { label: "English Proficiency", detail: "C1 Level (IELTS 7.0 / TOEFL 95) for International Master's." },
        { label: "GRE Requirement", detail: "Mandatory for non-EU applicants in specific technical Master's." }
      ]
    }
  ];

  const stats = [
    { icon: <Binary />, label: "TU9 Member", value: "Elite", color: "bg-cyan-800" },
    { icon: <Microscope />, label: "Research Focus", value: "Cyber/AI", color: "bg-teal-700" },
    { icon: <TrendingUp />, label: "Start-up Hub", value: "#1 DE", color: "bg-sky-800" },
    { icon: <Award />, label: "Engineering Rank", value: "Top 100", color: "bg-slate-800" }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-500 pb-20 overflow-hidden">
      ...
      <motion.div style={{ opacity, scale }} className="relative h-[100vh] w-full overflow-hidden">
        ...
        >
          <img 
            src={universityImages.tuDarmstadt}
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-r from-cyan-900/40 via-transparent to-teal-900/30 opacity-70" />
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.8 }} className="relative">
            ...
            <h1 className="text-6xl md:text-8xl font-black text-white mb-4 tracking-tight">Darmstadt</h1>
            <p className="text-xl md:text-2xl text-white/90 font-medium max-w-2xl mx-auto">First Independent University in Germany</p>
          </motion.div>
        </div>
          ...
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-32 space-y-6">
              <motion.div variants={itemVariants} className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl p-8 rounded-[3rem] border border-cyan-100 dark:border-slate-800 shadow-2xl relative overflow-hidden group">
                ...
                <motion.div variants={floatVariants} animate="animate" className="w-16 h-16 bg-gradient-to-br from-cyan-800 to-teal-950 rounded-2xl mb-6 flex items-center justify-center shadow-xl rotate-3 relative">
                  <Landmark size={32} className="text-white relative z-10" />
                </motion.div>
                <h1 className="text-4xl font-black text-slate-900 dark:text-white mb-2 leading-tight">TU <span className="text-cyan-700 italic block">Darmstadt</span></h1>
                ...
                <div className="space-y-4 pt-6 border-t border-cyan-50 dark:border-slate-800">
                  <div className="flex justify-between items-center group/deadline">
                    <span className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 group-hover/deadline:text-cyan-600 transition-colors">
                      <Clock size={14} className="text-cyan-600" /> Winter Intake
                    </span>
                    <span className="font-bold dark:text-white">Sept 01</span>
                  </div>
                  <div className="flex justify-between items-center group/deadline">
                    <span className="text-xs font-black uppercase text-slate-400 tracking-widest flex items-center gap-2 group-hover/deadline:text-cyan-600 transition-colors">
                      <Clock size={14} className="text-cyan-600" /> Summer Intake
                    </span>
                    <span className="font-bold dark:text-white">March 01</span>
                  </div>
                </div>
              </motion.div>

              <motion.div variants={itemVariants} className="relative bg-gradient-to-br from-cyan-900 via-teal-900 to-slate-950 p-8 rounded-[2.5rem] text-white shadow-2xl overflow-hidden group">
                ...
                <p className="text-sm font-medium leading-relaxed italic relative z-10">
                  "As a member of TU9, we focus on engineering excellence and are world-renowned for our contributions to Electrical Engineering and Computer Science."
                </p>
              </motion.div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="grid md:grid-cols-2 gap-6">
              {sections.map((section, idx) => (
                <motion.div key={idx} variants={itemVariants} whileHover={{ y: -8, rotateY: 2 }} className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-cyan-50 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden group">
                  <div className="absolute inset-0 bg-gradient-to-br from-cyan-500/0 via-teal-500/0 to-slate-500/0 group-hover:from-cyan-500/5 group-hover:via-teal-500/5 group-hover:to-slate-500/5 transition-all duration-500" />
            ...
            <motion.div variants={itemVariants} className="bg-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden border border-slate-800">
              ...
              <div className="grid md:grid-cols-2 gap-4 relative z-10">
                {["TUCaN Portal Application", "Certified Degree Certificates", "DSH/TestDaF/Goethe Cert", "Financial Proof (Blocked Account)", "Module Catalog (For Masters)", "APS Certificate (Selected Countries)"].map((item, i) => (
                  <motion.div key={i} whileHover={{ x: 10, scale: 1.02 }} className="flex items-center gap-3 bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-white/10 hover:border-cyan-500/50 transition-all group">
                    <CheckCircle2 size={18} className="text-cyan-400 group-hover:text-teal-400" />
            ...
            <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-cyan-50 dark:border-slate-800 shadow-sm relative overflow-hidden">
              ...
              <div className="grid md:grid-cols-2 gap-8 items-center relative z-10">
                <div className="space-y-4">
                  {["Technical Excellence", "Problem-Solving Mindset", "Research Initiative", "Systemic Thinking"].map((rule, i) => (
                    <motion.div key={i} whileHover={{ x: 10 }} className="flex items-center gap-4 group">
                      <div className="w-2 h-2 rounded-full bg-cyan-600 group-hover:bg-teal-400" />
                      <span className="text-md font-bold text-slate-700 dark:text-slate-300 group-hover:text-cyan-700">{rule}</span>
                    </motion.div>
                  ))}
                </div>
                <motion.div whileHover={{ scale: 1.05 }} className="bg-gradient-to-br from-cyan-900 via-teal-950 to-slate-950 p-8 rounded-[2rem] text-white shadow-2xl relative overflow-hidden">
                  <p className="text-xs font-black uppercase tracking-widest mb-2 opacity-90 relative z-10">Semester Fee</p>
                  <h4 className="text-xl font-black mb-2 relative z-10 flex items-center gap-2"><Award size={24} /> No Tuition Fees</h4>
                  <p className="text-sm font-medium leading-relaxed relative z-10">
                    Darmstadt charges no tuition for most programs. Only a semester contribution of approx. €280 is required, including the Semesterticket for transport.
                  </p>
                </motion.div>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants} className="pt-10 flex flex-col items-center gap-6">
              <motion.button 
                ...
                className="group relative px-12 py-6 bg-gradient-to-r from-cyan-800 via-teal-950 to-cyan-800 text-white rounded-full font-black uppercase tracking-[0.2em] text-sm transition-all shadow-2xl overflow-hidden"
              >
                <motion.span initial={{ x: "-100%" }} whileHover={{ x: "100%" }} transition={{ duration: 0.6 }} className="absolute inset-0 bg-gradient-to-r from-teal-600 via-cyan-700 to-teal-600" />
                <span className="relative z-10 flex items-center gap-2"><Sparkles size={18} /> Apply to TU Darmstadt</span>
              </motion.button>
              <motion.p animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }} className="text-slate-400 font-bold text-xs uppercase tracking-widest flex items-center gap-2">
                <Clock size={14} className="text-cyan-600" /> Winter 2026 Portal Opening March
              </motion.p>
            </motion.div>
