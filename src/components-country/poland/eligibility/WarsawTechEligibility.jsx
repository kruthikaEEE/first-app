...
const WarsawTechEligibility = () => {
  ...
  const sections = [
    { title: "Academic Eligibility", icon: <BookOpen className="text-slate-600" />, content: [
      { label: "Undergraduate Programs", detail: "Completion of 12 years with strong STEM foundation. Mathematics and Physics prerequisites essential." },
      { label: "Core Preparation", detail: "Excellent in Mathematics, Physics, Computer Science. Hands-on project experience valued." },
      { label: "Graduate / Master's Programs", detail: "Bachelor's in engineering with GPA 3.0+. Technical portfolio and research experience preferred." }
    ]},
    { title: "Language & Tests", icon: <Globe2 className="text-slate-600" />, content: [
      { label: "Language Requirement", detail: "B2 Polish for Polish programs. Most engineer programs taught in English; IELTS 6.5+, TOEFL 90+." },
      { label: "Entrance Exam", detail: "Competitive entrance exam in Mathematics and Physics. Technical problem-solving assessed." },
      { label: "Graduate Assessment", detail: "GRE recommended. Technical portfolio, GitHub, or published work valued." }
    ]}
  ];

  const stats = [
    { icon: <Users />, label: "Acceptance Rate", value: "≈22%", color: "bg-slate-600" },
    { icon: <TrendingUp />, label: "Avg Starting Salary", value: "€50k+", color: "bg-green-500" },
    { icon: <Award />, label: "Top Fields", value: "Engineering • IT • Architecture", color: "bg-amber-500" },
    { icon: <Globe2 />, label: "Founded", value: "1826", color: "bg-slate-500" }
  ];

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-500 pb-20 overflow-hidden">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <motion.div animate={{ x: [0, 100, 0], y: [0, -100, 0], scale: [1, 1.2, 1], rotate: [0, 90, 0] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }} className="absolute top-20 right-20 w-96 h-96 bg-slate-600/10 dark:bg-slate-600/5 rounded-full blur-3xl" />
        <motion.div animate={{ x: [0, -150, 0], y: [0, 100, 0], scale: [1, 1.3, 1], rotate: [0, -90, 0] }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} className="absolute bottom-20 left-20 w-[500px] h-[500px] bg-slate-500/10 dark:bg-slate-500/5 rounded-full blur-3xl" />
      </div>
      ...
      <motion.div style={{ opacity, scale }} className="relative h-[70vh] w-full overflow-hidden">
        <motion.div initial={{ scale: 1.3, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1.5, ease: "easeOut" }} style={{ x: mousePosition.x, y: mousePosition.y }} className="absolute inset-0">
          <img src="/assets/hero-poland.jpg" alt="Warsaw University of Technology" className="w-full h-full object-cover" />
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-slate-900/50 via-transparent to-slate-800/30 opacity-70" />
        </motion.div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
          <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.8, delay: 0.8 }} className="relative">
            <motion.div animate={{ boxShadow: ["0 0 20px rgba(71, 85, 105, 0.3)", "0 0 40px rgba(71, 85, 105, 0.6)", "0 0 20px rgba(71, 85, 105, 0.3)"] }} transition={{ duration: 2, repeat: Infinity }} className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md border border-white/30 text-white px-6 py-3 rounded-full mb-6 font-bold"><Sparkles size={16} className="text-slate-400" />Engineering Excellence in Central Europe</motion.div>
            <h1 className="text-5xl md:text-7xl font-black text-white mb-4 tracking-tight">Warsaw University of Technology</h1>
            <p className="text-xl md:text-2xl text-white/90 font-medium max-w-2xl">Poland's Top Engineering & Innovation Hub</p>
          </motion.div>
        </div>

        <div className="absolute top-32 left-6 lg:left-20 z-20">
          <motion.button onClick={() => navigate(-1)} whileHover={{ scale: 1.05, x: -5 }} whileTap={{ scale: 0.95 }} className="group flex items-center gap-2 bg-white/10 backdrop-blur-xl border border-white/30 text-white px-6 py-3 rounded-full font-bold hover:bg-white hover:text-slate-800 transition-all shadow-2xl relative overflow-hidden">
          ...
          <div className="lg:col-span-1">
            <div className="lg:sticky lg:top-32 space-y-6">
              <motion.div variants={itemVariants} whileHover={{ scale: 1.02 }} className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl p-8 rounded-[3rem] border border-gray-200 dark:border-slate-800 shadow-2xl relative overflow-hidden group">
                ...
                <motion.div variants={floatVariants} animate="animate" className="w-16 h-16 bg-gradient-to-br from-slate-700 to-slate-500 rounded-2xl mb-6 flex items-center justify-center shadow-xl rotate-3 relative">
                ...
                <h1 className="text-4xl font-black text-gray-900 dark:text-white mb-2 leading-tight">Warsaw Tech <span className="text-slate-700 italic block">Poland</span></h1>
                <p className="text-gray-500 dark:text-slate-400 font-medium mb-6 flex items-center gap-2"><Star size={16} className="text-amber-400 fill-amber-400" />Top Engineering • Warsaw, Poland</p>
                <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-slate-800">
                  {[{ label: "Fall Intake", date: "September" }, { label: "Spring Intake", date: "February" }].map((deadline, idx) => (
                    <motion.div key={idx} whileHover={{ x: 5 }} className="flex justify-between items-center group/deadline">
                      <span className="text-xs font-black uppercase text-gray-400 tracking-widest flex items-center gap-2 group-hover/deadline:text-slate-600 transition-colors"><Clock size={14} className="text-slate-600" /> {deadline.label}</span>
              ...
              <motion.div variants={itemVariants} whileHover={{ scale: 1.02 }} className="relative bg-gradient-to-br from-slate-700 via-slate-600 to-slate-800 p-8 rounded-[2.5rem] text-white shadow-2xl overflow-hidden group">
                ...
                <Lightbulb className="mb-4 text-slate-200" size={32} />
                <p className="text-sm font-medium leading-relaxed italic relative z-10">"Warsaw Tech launches Polish engineering leaders. Master cutting-edge methods in Eastern Europe's innovation capital."</p>
              </motion.div>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="grid md:grid-cols-2 gap-6">
              {sections.map((section, idx) => (
                <motion.div key={idx} variants={itemVariants} whileHover={{ y: -8, rotateY: 2 }} className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all relative overflow-hidden group">
                  <div className="absolute inset-0 bg-gradient-to-br from-slate-500/0 via-slate-500/0 to-slate-500/0 group-hover:from-slate-500/5 group-hover:via-slate-500/5 group-hover:to-slate-500/5 transition-all duration-500" />
                  ...
                  <div className="space-y-4 relative z-10">
                    {section.content.map((item, i) => (
                      <motion.div key={i} whileHover={{ x: 5 }} className="group/item">
                        <p className="text-[10px] font-black uppercase text-gray-400 tracking-tighter group-hover/item:text-slate-600 transition-colors flex items-center gap-2"><Zap size={10} className="opacity-0 group-hover/item:opacity-100 transition-opacity" />{item.label}</p>
            ...
            <motion.div variants={itemVariants} className="bg-gradient-to-br from-gray-900 via-slate-900 to-gray-900 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 text-white p-10 rounded-[3rem] shadow-2xl relative overflow-hidden border border-slate-800">
              ...
              <h2 className="text-3xl font-black mb-8 flex items-center gap-3 relative z-10">
                <motion.div animate={{ rotate: [0, 10, -10, 0] }} transition={{ duration: 2, repeat: Infinity }}><FileText className="text-slate-500" /></motion.div>
                Application Checklist
              </h2>
              <div className="grid md:grid-cols-2 gap-4 relative z-10">
                {["Online application via PW portal", "High school/bachelor transcripts", "2 Technical references", "English proficiency (IELTS/TOEFL)", "Technical portfolio or GitHub", "Engineering interest statement"].map((item, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} whileHover={{ x: 10, scale: 1.02 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} className="flex items-center gap-3 bg-white/5 backdrop-blur-sm p-4 rounded-2xl border border-white/10 hover:border-slate-500/50 hover:bg-white/10 transition-all cursor-pointer group">
                    <motion.div whileHover={{ rotate: 360, scale: 1.2 }} transition={{ duration: 0.5 }}><CheckCircle2 size={18} className="text-slate-500 group-hover:text-green-400 transition-colors" /></motion.div>
            ...
            <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-gray-100 dark:border-slate-800 shadow-sm relative overflow-hidden">
              <motion.div animate={{ scale: [1, 1.2, 1], rotate: [0, 90, 0] }} transition={{ duration: 10, repeat: Infinity }} className="absolute -top-10 -right-10 w-40 h-40 bg-slate-100 dark:bg-slate-900/20 rounded-full blur-3xl" />
              <div className="flex items-center gap-3 mb-8 relative z-10">
                <motion.div whileHover={{ rotate: 180, scale: 1.1 }} transition={{ type: "spring", stiffness: 200 }} className="p-4 bg-slate-50 dark:bg-slate-900/30 rounded-3xl"><Target className="text-slate-600" size={28} /></motion.div>
                <h2 className="text-3xl font-black dark:text-white tracking-tight">Technical Excellence</h2>
              </div>
              <div className="grid md:grid-cols-2 gap-8 items-center relative z-10">
                <div className="space-y-4">
                  {["Exceptional STEM skills", "Problem-solving capability", "Technical project experience", "Innovation & practical mindset"].map((rule, i) => (
                    <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} whileHover={{ x: 10 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }} className="flex items-center gap-4 group">
                      <motion.div whileHover={{ scale: 1.5, rotate: 180 }} className="w-2 h-2 rounded-full bg-slate-600 group-hover:bg-green-500 transition-colors" />
                      <span className="text-md font-bold text-gray-700 dark:text-slate-300 group-hover:text-slate-600 dark:group-hover:text-slate-400 transition-colors">{rule}</span>
                    </motion.div>
                  ))}
                </div>
                <motion.div whileHover={{ scale: 1.05, rotate: 1 }} className="bg-gradient-to-br from-slate-600 via-slate-700 to-slate-800 p-8 rounded-[2rem] text-white shadow-2xl shadow-slate-500/30 relative overflow-hidden">
                  ...
                  <h4 className="text-xl font-black mb-2 relative z-10 flex items-center gap-2"><Award size={24} />Competitive Programs</h4>
                  <p className="text-sm font-medium leading-relaxed relative z-10">EU/EEA: €3,000-5,000/year. Non-EU: €5,000-8,000/year. Scholarships for engineering excellence available.</p>
                </motion.div>
              </div>
            </motion.div>

            <motion.div variants={itemVariants} className="pt-10 flex flex-col items-center gap-6">
              <motion.button onClick={() => navigate('/enquiry')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="group relative px-12 py-6 bg-gradient-to-r from-slate-700 via-slate-600 to-slate-700 text-white rounded-full font-black uppercase tracking-[0.2em] text-sm transition-all shadow-2xl shadow-slate-500/30 overflow-hidden">
                <motion.span className="absolute inset-0 bg-gradient-to-r from-slate-600 via-slate-500 to-slate-600" initial={{ x: "-100%" }} whileHover={{ x: "100%" }} transition={{ duration: 0.6 }} />
              ...
              <motion.p animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 2, repeat: Infinity }} className="text-gray-400 font-bold text-xs uppercase tracking-widest flex items-center gap-2"><Clock size={14} className="text-slate-500" />Competitive engineering programs</motion.p>
            </motion.div>
          </div>
        </div>
      </motion.div>
...
export default WarsawTechEligibility;
